...
if !exists('g:fops_fstack_table')
    let g:fops_fstack_table = {}
endif

" Takes in a buffer ID and returns a fstack object if the buffer is found
" within the table. Otherwise, v:null is returned.
function! fops#fstack_table#lookup(buffer_id) abort
    let l:bid = a:buffer_id
    return get(g:fops_fstack_table, l:bid, v:null)
endfunction

" Works similarly to `fops#fstack_table#lookup()`, except if the given buffer
" ID currently has no entry in the global table, one is created and inserted
" into the table. The object is returned.
function! fops#fstack_table#get(buffer_id) abort
    let l:bid = a:buffer_id
    let l:fstack = fops#fstack_table#lookup(l:bid)
    if l:fstack is v:null
        ...
        let l:fstack = fops#fstack#new()
        call fops#fstack#set_buffer(l:fstack, l:bid)

        " insert into the table
        let g:fops_fstack_table[l:bid] = l:fstack
    endif
